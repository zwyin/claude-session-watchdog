...
| 模型 | GLM-5.1（通过 GLM Coding Plan 代理） |
...
```
Claude Code CLI ──HTTP/SSE──→ GLM Coding Plan 代理 ──→ GLM-5.1 模型
```
