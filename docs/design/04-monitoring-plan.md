...
| `idle_decision` | 空闲会话，等待人工决策 | 自动分类 |
| `idle_complete` | 空闲会话，任务已完成 | 自动分类 |
| `idle_unknown` | 空闲会话，原因不明 | 自动分类 |
...
| model | string | 模型名（从状态栏动态提取，可通过 MODEL_NAME 环境变量覆盖） |
...
空闲会话（5 分钟无输出但不判定为卡住）→ 关键字分类 + 可选 LLM 语义分析 → 分类通知。
