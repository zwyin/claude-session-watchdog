...
"""],
    ...
    def test_path_with_dots(self):
        result = subprocess.run(
            ["python3", "-c", """
import re
workdir = '/home/user/projects/claude-session-watchdog'
home = '/home/user'
rel = workdir.replace(home + '/', '')
encoded = re.sub(r'[^a-zA-Z0-9]+', '-', rel).strip('-')
print(encoded)
"""],
        ...
        self.assertEqual(result.stdout.strip(), "projects-claude-session-watchdog")

    def test_path_with_spaces(self):
        result = subprocess.run(
            ["python3", "-c", """
import re
workdir = '/home/user/my projects/test app'
home = '/home/user'
rel = workdir.replace(home + '/', '')
encoded = re.sub(r'[^a-zA-Z0-9]+', '-', rel).strip('-')
print(encoded)
...
class TestFormatDetection(unittest.TestCase):
    ...
    def test_url_anthropic_detected(self):
        from llm_utils import is_anthropic_format
        self.assertTrue(is_anthropic_format("https://api.example.com/anthropic"))

    def test_url_openai_default(self):
        from llm_utils import is_anthropic_format
        self.assertFalse(is_anthropic_format("https://api.example.com/v4"))
...
class TestCallLlmApiConstruction(unittest.TestCase):
    ...
    def test_anthropic_format_url(self):
        ...
        try:
            self.call_llm_fn("https://api.example.com/anthropic", "key", "model", "test", fmt="anthropic")
        except Exception:
            pass
        finally:
            urllib.request.Request = orig_req
        self.assertEqual(captured['url'], "https://api.example.com/anthropic/v1/messages")
    ...
    def test_openai_format_url(self):
        ...
        urllib.request.Request = mock_request
        try:
            self.call_llm_fn("https://api.example.com/v4", "key", "model", "test", fmt="openai")
        except Exception:
            pass
        finally:
            urllib.request.Request = orig_req
        self.assertEqual(captured['url'], "https://api.example.com/v4/chat/completions")
...
class TestVersionAndConfig(unittest.TestCase):
    ...
    def test_version_current(self):
        with open(WATCHDOG_SCRIPT) as f:
            content = f.read()
        self.assertIn('VERSION=\"2.0.7\"', content)
